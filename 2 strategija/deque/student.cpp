...
void rūšiuoja_ir_rašo_failus(std::deque<Student>& students) {
    ...
    // Sukurkime iteratorių, kuris parodytų, kur baigiasi geri mokiniai ir prasideda blogi mokiniai
    auto partition_point = std::partition(students.begin(), students.end(), arGerasStudentas);

    // Perkeliame blogus mokinius į naują konteinerį
    blogiMokiniai.insert(blogiMokiniai.end(), partition_point, students.end());
