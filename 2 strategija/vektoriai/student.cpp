...
void rūšiuoja_ir_rašo_failus(std::vector<Student>& students) {
    ...
    // Išskiriame blogus mokinius į naują konteinerį ir juos ištriname iš bendro konteinerio
    auto partition_point = std::remove_if(tempStudents.begin(), tempStudents.end(), arGerasStudentas);
    std::vector<Student> blogiMokiniai(partition_point, tempStudents.end());
    tempStudents.erase(partition_point, tempStudents.end());
