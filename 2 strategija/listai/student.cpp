...
void rūšiuoja_ir_rašo_failus(std::list<Student>& students) {
    ...
    for (auto iter = students.begin(); iter != students.end();) {
        if (!arGerasStudentas(*iter)) { // Jei studentas vargšiukas
            bloguciai.push_back(*iter); // Įkeliam į naująjį konteinerį "bloguciai"
            iter = students.erase(iter); // Ištrinam iš bendro konteinerio
        } else {
            ++iter; // Pajudinam į kitą elementą
        }
    }

    // Čia likę tik geri mokiniai bendrame konteineryje
