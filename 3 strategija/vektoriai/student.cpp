...
void rūšiuoja_ir_rašo_failus(const std::vector<Student>& students) {
    ...
    if (kietiakai.is_open()) {
        ...
        for (auto it = tempStudents.begin(); it != partition_point; ++it) {
    ...
    if (bloguciai.is_open()) {
        ...
        for (auto it = partition_point; it != tempStudents.end(); ++it) {
            const auto& student = *it;
