...
void rūšiuoja_ir_rašo_failus(std::list<Student>& students) {
    std::list<Student> bloguciai;

    // Suskirstome studentus į du segmentus: geri ir blogi
    auto dalijimo_taskas = std::stable_partition(students.begin(), students.end(), arGerasStudentas);

    // Išskyriame blogus studentus ir juos ištriname iš bendro sąrašo
    std::move(std::next(dalijimo_taskas), students.end(), std::back_inserter(bloguciai));
    students.erase(dalijimo_taskas, students.end());

    // Rūšiuojame gerus studentus pagal vidurkį
    students.sort(lygintiPagalVidurki);

    // Rašome gerų studentų failą
    std::ofstream geruciaiFailas("geruciai.txt");
    if (geruciaiFailas.is_open()) {
        geruciaiFailas << std::left << std::setw(20) << "Vardas" << std::setw(20) << "Pavarde" << std::setw(10) << "Galutinis" << std::endl;
        geruciaiFailas << "--------------------------------------------------------------------------------" << std::endl;

        for (const auto& student : students) {
            geruciaiFailas << std::left << std::setw(20) << student.vardas << std::setw(20) << student.pavarde << std::setw(10) << skaiciuotiVidurki(student.pazymiai) << std::endl;
        }
        geruciaiFailas.close();
    } else {
        std::cerr << "Nepavyko atidaryti failo geruciai.txt";
    }

    // Rašome blogų studentų failą
    std::ofstream bloguciaiFailas("bloguciai.txt");
    if (bloguciaiFailas.is_open()) {
        bloguciaiFailas << std::left << std::setw(20) << "Vardas" << std::setw(20) << "Pavarde" << std::setw(10) << "Galutinis" << std::endl;
        bloguciaiFailas << "--------------------------------------------------------------------------------" << std::endl;

        for (const auto& student : bloguciai) {
            bloguciaiFailas << std::left << std::setw(20) << student.vardas << std::setw(20) << student.pavarde << std::setw(10) << skaiciuotiVidurki(student.pazymiai) << std::endl;
        }
