...
void rūšiuoja_ir_rašo_failus(std::deque<Student>& students) {
    // Sukurkime naują konteinerį tik su blogais mokiniais
    std::deque<Student> blogiMokiniai;

    // Ištriname blogus mokinius iš bendro konteinerio ir perkeliame į naują
    auto iter = std::remove_if(students.begin(), students.end(), [&](const Student& s) {
        if (!arGerasStudentas(s)) {
            blogiMokiniai.push_back(s);
            return true;
        }
        return false;
    });

    // Ištriname blogus mokinius iš bendro konteinerio
    students.erase(iter, students.end());

    // Rašome gerus mokinius į atskirą failą
    std::ofstream geruciaiFailas("geruciai.txt");
    if (geruciaiFailas.is_open()) {
        geruciaiFailas << std::left << std::setw(20) << "Vardas" << std::setw(20) << "Pavarde";
        geruciaiFailas << std::left << std::setw(10) << "Galutinis" << std::endl;
        geruciaiFailas << "--------------------------------------------------------------------------------------" << std::endl;

        for (const auto& student : students) {
            geruciaiFailas << std::left << std::setw(20) << student.name << std::setw(20) << student.sur;
            geruciaiFailas << std::left << std::setw(10) << skaiciuotiVidurki(student.pazymiai) << std::endl;
        }
        geruciaiFailas.close();
    } else {
        std::cerr << "Nepavyko atidaryti failo geruciai.txt" << std::endl;
    }

    // Rašome blogus mokinius į atskirą failą
    std::ofstream bloguciaiFailas("bloguciai.txt");
    if (bloguciaiFailas.is_open()) {
        bloguciaiFailas << std::left << std::setw(20) << "Vardas" << std::setw(20) << "Pavarde";
        bloguciaiFailas << std::left << std::setw(10) << "Galutinis" << std::endl;
        bloguciaiFailas << "--------------------------------------------------------------------------------------" << std::endl;

        for (const auto& student : blogiMokiniai) {
            bloguciaiFailas << std::left << std::setw(20) << student.name << std::setw(20) << student.sur;
            bloguciaiFailas << std::left << std::setw(10) << skaiciuotiVidurki(student.pazymiai) << std::endl;
        }
        bloguciaiFailas.close();
    } else {
        std::cerr << "Nepavyko atidaryti failo bloguciai.txt" << std::endl;
    }
}
